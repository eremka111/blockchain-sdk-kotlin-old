...
@JvmInline
value class GasEstimationMultiplier private constructor(val value: Double) {
        ...
        fun from(multiplier: Double): Result<GasEstimationMultiplier> {
            return when {
                multiplier < 1.0 || multiplier > 2.0 -> {
                    Result.failure(IllegalArgumentException("Invalid gas estimation multiplier, expected a value between [1.0, 2.0]"))
                }
                ...
                multiplier.toString().let { str ->
                    ...
                    Result.failure(IllegalArgumentException("Invalid gas estimation multiplier precision, maximum allowed precision is $MaxPrecision"))
                }
